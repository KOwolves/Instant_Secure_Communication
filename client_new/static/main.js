document.addEventListener('DOMContentLoaded', function() {
            ...
            if (panelId === 'contacts') {
                ...
                .then(data => {
                    if (data.success) {
                        ...
                        const contactList = document.querySelector('.contacts .list');
                        if (contactList) {
                            contactList.innerHTML = '<div class="error-message">获取好友列表失败，请刷新页面重试</div>';
                            // 添加重试按钮
                ...
                .catch(error => {
                    ...
                    const contactList = document.querySelector('.contacts .list');
                    if (contactList) {
                        contactList.innerHTML = '<div class="error-message">网络错误，无法获取好友列表</div>';
                        // 添加重试按钮
                        const retryButton = document.createElement('button');
                        retryButton.className = 'retry-btn';
                        retryButton.textContent = '重试';
    ...
    function safeGetFriendData(friend) {
        try {
            ...
            const userId = friend.user_id || friend.UserID || '';
            
            // 收集可能的IP地址和端口信息，允许多种字段名格式
            const ipAddress = friend.ip_address || friend.IPAddress || friend.ip || '';
            const p2pPort = friend.p2p_port || friend.P2PPort || friend.port || 0;
            ...
            console.error('处理好友数据时出错:', error, friend);
    ...
    function simulateContactItemClick(contactItem) {
        try {
            ...
            // 安全地检查好友是否在线
            const isOnline = window.onlineFriendsList.some(f => {
                const friendName = f && (f.username || f.name || '');
