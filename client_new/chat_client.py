...
class ChatClient:
    ...
    def get_public_key_from_server(self, username):
        ...
        if username in self.online_friends_info:
            return self.online_friends_info[username]["public_key"]
    ...
    def _handle_p2p_received_raw_data(self, peer_username, peer_public_key_pem, raw_data_bytes, sid=None):
        ...
        try:
            ...
            # 检查消息类型，可以是普通聊天消息或隐写图片消息
            message_type = received_payload.get("type", "chat_message")  # 默认为普通聊天消息
            ...
            if message_type == "chat_message":
                ...
                if decrypted_message_bytes is None:
                    logger.error(f"无法解密来自 {peer_username} 的聊天消息，可能被篡改或密钥错误。")
                    return
                ...
                if sid:
                    self.socketio_instance.emit('receive_message',
                                                ...
                                                room=sid)
                    logger.debug(f"通过SocketIO向SID {sid} 推送聊天消息。")
                else:
                    logger.warning(f"没有可用的SocketIO SID，聊天消息无法推送到浏览器。")
    ...
    def send_p2p_message(self, recipient_username, message):
        ...
        try:
            ...
            if self.p2p_manager.send_p2p_raw_data(recipient_username,
                ...
                logger.info(f"已发送加密P2P聊天消息给 {recipient_username}.")
                return True
            else:
                logger.error(f"通过P2PManager发送聊天消息给 {recipient_username} 失败。")
                return False
    ...
    def send_steg_image_message(self, recipient_username, image_file_bytes, hidden_message_text):
        ...
        friend_ip = friend_info["ip"]
        friend_port = friend_info["port"]
        friend_public_key_pem = friend_info["public_key"]
