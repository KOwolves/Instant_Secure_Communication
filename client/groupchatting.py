...
def  start_group_chat(gnum,s,myId):
#监听函数，while true之下一直监听，收到消息就msgshow显示到界面之上
    def lis(s):
        #my = [s]
        while True:
            #r, w, e = select.select(my, [],my)
            #if s in r:
                try:
...
#发送消息，显示到界面之上
        ...
        info_send = '22'+'|'+ gnum + '@@@' + myId+'@@@'+info
        s.send(info_send.encode())
        msgshow(myId, info)
    ...
    button_send = Button(f_floor, text='Send', bd=0, bg='YellowGreen', width=8, font='等线',
                         command=lambda: talk(s, myId))  # 按钮分区中创建按钮并绑定发送消息函数
