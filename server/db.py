...
class DatabaseManager:
    ...
    def clear_online_status(self, user_id):
        ...
        # 检查用户ID是否有效
        if user_id is None:
            logger.warning("DatabaseManager: 尝试清除在线状态时用户ID为None，跳过操作。")
            return False
    ...
    def get_online_friends_info(self, user_id):
        """获取指定用户所有在线好友的信息（UserID、IP、端口、公钥）。"""
        conn = None
        online_friends = []
        try:
            conn = self._get_connection()
            ...
            sql = """
                SELECT
                    ...
                    u.PublicKey
                FROM Friendships fs
                JOIN OnlineStatus os ON (
                    (fs.UserID = ? AND os.UserID = fs.FriendID) OR   -- 当前用户是 UserID，查找 FriendID
                    (fs.FriendID = ? AND os.UserID = fs.UserID)      -- 当前用户是 FriendID，查找 UserID
                )
            ...
            for row in cursor.fetchall():
                online_friends.append({
                    ...
                    "ip": row.IPAddress,
                    "port": row.P2PPort,
                    "public_key_pem": row.PublicKey  # 使用public_key_pem作为字段名
                })
    ...
    def get_all_friends_info(self, user_id):
        """获取指定用户的所有好友信息（UserID、用户名、IP、端口，包括在线和离线）。"""
        conn = None
        all_friends = []
        try:
            conn = self._get_connection()
            cursor = conn.cursor()
            # 查询作为 UserID 或 FriendID 的好友，并获取在线状态信息
            sql = """
                SELECT
                    u.UserID,
                    u.Username,
                ...
                LEFT JOIN OnlineStatus os ON u.UserID = os.UserID
                WHERE (fs.UserID = ? OR fs.FriendID = ?)
            """
            ...
            for row in cursor.fetchall():
                friend_info = {
                    "user_id": row.UserID,
                    "username": row.Username,
                    "ip_address": row.IPAddress if row.IPAddress else None,
                    "p2p_port": row.P2PPort if row.P2PPort else None
                }
