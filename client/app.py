...
from config import MAX_HEARTBEAT_INTERVAL, XUNFEI_APPID, XUNFEI_API_KEY, XUNFEI_API_SECRET

# 导入自定义模块
from chat_client import ChatClient
from utils.ASR import XunfeiASR, speech_to_text
...
# 初始化讯飞ASR服务
asr_service = XunfeiASR(XUNFEI_APPID, XUNFEI_API_KEY, XUNFEI_API_SECRET)
...
def speech_to_text_api():
    ...
    try:
        if 'audio_file' not in request.files:
            logger.error("未找到上传的音频文件")
            return jsonify({'status': 'error', 'message': '未找到上传的音频文件'}), 400
        ...
        if not audio_file or not audio_file.filename:
            logger.error("音频文件无效")
            return jsonify({'status': 'error', 'message': '音频文件无效'}), 400
        
        # 确保上传目录存在
        upload_dir = os.path.join(os.path.dirname(os.path.dirname(__file__)), 'uploads', 'temp_audio')
        os.makedirs(upload_dir, exist_ok=True)
        
        # 保存文件
        timestamp = int(time.time() * 1000)
        random_suffix = os.urandom(2).hex()
        filename = f"audio_{timestamp}_{random_suffix}.wav"
        filepath = os.path.join(upload_dir, filename)
        audio_file.save(filepath)
        
        logger.info(f"音频文件已保存: {filepath}")
        
        # 使用 ASR 模块进行语音识别（异步处理）
        def process_audio():
            try:
                # 进行语音识别
                ...
                if result and 'text' in result:
                    ...
                    with asr_results_lock:
                        asr_results[message_id] = result['text']
                    ...
                    logger.info(f"准备通过Socket.IO发送识别结果: {result_data}")
                    socketio.emit('asr_result', result_data)
                    logger.info(f"Socket.IO事件已发送")
                    
                    # 直接在响应中返回结果，确保前端能立即获取到
                    return result['text']
                else:
                    error_msg = result.get('error', '语音识别失败')
                    logger.error(f"语音识别返回无效结果: {error_msg}")
                ...
                logger.exception(f"语音识别处理异常: {str(e)}")
                socketio.emit('asr_result', {
                    'status': 'error',
                    'message_id': message_id,
                    'error': f"处理异常: {str(e)}"
                ...
                try:
                    ...
                    logger.warning(f"无法删除临时音频文件 {filepath}: {str(e)}")
        
        # 直接同步处理，不再使用后台任务
        text_result = process_audio()
        ...
        logger.exception(f"语音转文字处理异常: {str(e)}")
        return jsonify({'status': 'error', 'message': str(e)}), 500

# 获取语音识别结果
@app.route('/api/get_speech_text/<message_id>', methods=['GET'])
def api_get_speech_text(message_id):
    """获取指定消息ID的语音识别结果"""
    with asr_results_lock:
        if message_id in asr_results:
            result = asr_results[message_id]
            return jsonify({'status': 'success', 'text': result}), 200
        else:
            return jsonify({'status': 'error', 'message': '识别结果不存在或识别仍在进行中'}), 404

# SocketIO事件处理：语音识别请求
@socketio.on('request_speech_to_text')
def handle_speech_to_text(data):
    """处理WebSocket语音识别请求"""
    logger = logging.getLogger('flask_app')
    logger.info("接收到WebSocket语音识别请求")
    
    if 'message_id' not in data or 'audio_data' not in data:
        logger.error("请求参数不完整，需要message_id和audio_data字段")
        return
    
    message_id = data['message_id']
    audio_data = data['audio_data']  # Base64编码的音频数据
    
    try:
        # 解码Base64音频数据
        audio_bytes = base64.b64decode(audio_data.split(',')[1] if ',' in audio_data else audio_data)
        logger.info(f"已解码音频数据，大小: {len(audio_bytes)} 字节")
        
        # 保存到临时文件
        temp_dir = os.path.join(app.config['UPLOAD_FOLDER'], 'temp_audio')
        os.makedirs(temp_dir, exist_ok=True)
        
        temp_file_path = os.path.join(temp_dir, f"{message_id}.wav")
        with open(temp_file_path, 'wb') as f:
            f.write(audio_bytes)
        logger.info(f"已保存音频到临时文件: {temp_file_path}")
        
        # 异步处理语音识别
        def process_asr_result():
            try:
                logger.info(f"开始处理音频识别: {temp_file_path}")
                
                # 使用新的speech_to_text函数进行识别
                ...
                if result and 'text' in result:
                    ...
                    logger.info(f"已发送识别结果到客户端: {result['text'][:50]}...")
                else:
                    error_msg = result.get('error', '语音识别失败')
                    logger.error(f"语音识别返回无效结果: {error_msg}")
                    emit('asr_result', {
                ...
                logger.exception(f"WebSocket语音识别处理异常: {str(e)}")
                emit('asr_result', {
                    'status': 'error',
                    'message_id': message_id,
                    'error': str(e),
        ...
        # 启动异步线程处理
        thread = threading.Thread(target=process_asr_result)
        thread.daemon = True
        thread.start()
        logger.info(f"语音识别线程已启动，message_id: {message_id}")
        
    except Exception as e:
        logger.exception(f"WebSocket语音识别请求处理异常: {str(e)}")
        emit('asr_result', {
            'status': 'error',
            'message_id': message_id,
            'error': str(e),
