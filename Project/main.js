document.addEventListener('DOMContentLoaded', function() {
        ...
        if (e.target.classList.contains('accept-btn') && e.target.closest('.friend-item') && e.target.closest('.search-result')) {
            ...
            if (requestsContent) {
                ...
                requestItem.innerHTML = `
                    <div class="avatar">
                        ${generateAvatar(friendName)}
                    </div>
                    <div class="info">
                        <div class="name">${friendName}</div>
                        <div class="message">请求添加您为好友</div>
                    </div>
                    <div class="actions">
    ...
    document.addEventListener('click', function(e) {
        ...
        if (e.target.classList.contains('delete-btn') && e.target.closest('.friend-item')) {
            ...
            if (confirm(`确定要删除好友 "${friendName}" 吗？`)) {
                ...
                contactItems.forEach(item => {
                    if (item.querySelector('.name').textContent === friendName) {
                        item.remove();
                    }
                });
                ...
                if (chatHistory[friendName]) {
                    delete chatHistory[friendName];
                }
                
                // 5. 如果当前正在与该好友聊天，清空聊天窗口
                const currentChatName = document.querySelector('.chat-window .title').textContent;
                if (currentChatName === friendName) {
                    document.querySelector('.messages').innerHTML = '<div class="date">今天</div>';
                    document.querySelector('.chat-window .title').textContent = '';
                }
    ...
    // 添加联系人到通讯录
    function addContactToList(name, avatar) {
        const contactList = document.querySelector('.contacts .list');
        
        // 如果名称中包含"搜索结果:"，则去除
        if (name.includes('搜索结果:')) {
            name = name.replace('搜索结果: ', '');
        }
        
        console.log("添加联系人:", name); // 调试信息
        
        // 检查是否已存在
        const existingContacts = contactList.querySelectorAll('.contact-item');
        for (let i = 0; i < existingContacts.length; i++) {
            if (existingContacts[i].querySelector('.name').textContent === name) {
                console.log("联系人已存在，不重复添加");
                return; // 已存在，不重复添加
            }
        }
        ...
        document.querySelectorAll('.chat-item').forEach(item => {
            ...
            clone.addEventListener('click', function() {
                ...
                // 确保聊天历史存在
                if (!chatHistory[itemName]) {
                    chatHistory[itemName] = [];
                }
