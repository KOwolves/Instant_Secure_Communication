...
def api_refresh_friends():
    ...
    if user_id:
        # 根据用户ID查找对应的客户端实例
        # 由于active_clients是以用户名为键存储的，需要遍历查找
        for username, client_instance in active_clients.items():
            if hasattr(client_instance, 'logged_in_user_id') and str(client_instance.logged_in_user_id) == str(user_id):
                client = client_instance
                logger.info(f"通过用户ID {user_id} 找到对应的客户端实例 {username}")
                break
    ...
    if client is None and 'username' in session:
        client = active_clients.get(session['username'])
        if client:
            logger.info(f"通过session找到对应的客户端实例 {session['username']}")
